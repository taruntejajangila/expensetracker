...
export class TwoFactorService {
  ...
  static async sendOTP(phone: string, otp: string): Promise<boolean> {
    try {
      ...
      // Use 2Factor.in standard SMS endpoint format
      // Format: https://2factor.in/API/V1/{API_KEY}/SMS/{PHONE}/{OTP}
      // Note: Template is configured in 2Factor.in dashboard, not in API URL
      // The API will use the default/approved template from your account settings
      const url = `/${TWO_FACTOR_API_KEY}/SMS/${cleanPhone}/${otp}`;
      
      logger.info(`Sending OTP via 2Factor.in SMS to ${phone}`);
      logger.debug(`2Factor.in API URL: ${TWO_FACTOR_BASE_URL}${url}`);
