...
export const up = async (pool: Pool): Promise<void> => {
  ...
  try {
    ...
    // Check if support_tickets table exists first
    ...
    if (!tableExists.rows[0].exists) {
      console.log('⚠️ Support tickets table does not exist, skipping migration');
      return;
    }
    ...
    try {
      ...
      console.log('⚠️ Ticket number column already exists or error:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('⚠️ Assigned_to column already exists or error:', (error as Error).message);
    }
    ...
    try {
      ...
      `);
      console.log('✅ Added is_admin_reply column');
    } catch (error) {
      console.log('⚠️ Is_admin_reply column already exists or error:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('⚠️ Updated_at column already exists or error:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('⚠️ Ticket_attachments table already exists or error:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('⚠️ Support_ticket_messages table already exists or error:', (error as Error).message);
    }
    ...
    try {
          await client.query(`
            ...
            $$ LANGUAGE plpgsql;
          `);
          console.log('✅ Created generate_ticket_number function');
        } catch (error) {
          console.log('⚠️ Generate_ticket_number function already exists or error:', (error as Error).message);
        }
    ...
    try {
      ...
      console.log('⚠️ Error updating tickets with ticket numbers:', (error as Error).message);
    }
    ...
    try {
      ...
      `);
      console.log('✅ Made ticket_number NOT NULL');
    } catch (error) {
      console.log('⚠️ Error making ticket_number NOT NULL:', (error as Error).message);
    }
