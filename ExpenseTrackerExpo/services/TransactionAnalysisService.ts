...
class TransactionAnalysisService {
  ...
  async getTransactions(userId?: string): Promise<Transaction[]> {
    try {
      ...
      console.log('🔍 Backend failed, throwing error');
      throw error;
    }
  }
  ...
  async generateLoanEMIReminders(userId?: string): Promise<Reminder[]> {
    try {
      ...
      const existingReminders = await this.getExistingReminders(userId);
      const existingLoanReminders = existingReminders.filter(r => r.isAutoGenerated && r.category === 'Loan/Debt Payments');
      ...
      for (const loan of loans) {
        ...
        if (daysUntilEMI <= 8 && daysUntilEMI >= 0) {
          // Check if we already have a reminder for this loan and EMI date
          const existingReminder = existingLoanReminders.find(r => 
            r.title.includes(loan.name) && 
            Math.abs(new Date(r.dueDate).getTime() - nextEMIDate.getTime()) < 24 * 60 * 60 * 1000 // Within 1 day
          );
          
          if (existingReminder) {
            console.log(`🏦 Skipping duplicate reminder for ${loan.name} (already exists)`);
  ...
  async generateSmartReminders(userId?: string): Promise<Reminder[]> {
    ...
    // Get existing reminders to check for duplicates
    const existingReminders = await this.getExistingReminders(userId);
    const existingSmartReminders = existingReminders.filter(r => r.isAutoGenerated && r.category !== 'Loan/Debt Payments');
    ...
    if (patterns.length > 0) {
      ...
      for (const pattern of patterns) {
        if (pattern.frequency === 'monthly') {
          ...
          // Check if we already have a reminder for this pattern and due date
          const existingReminder = existingSmartReminders.find(r => 
            r.category === pattern.category && 
            Math.abs(new Date(r.dueDate).getTime() - nextDueDate.getTime()) < 24 * 60 * 60 * 1000 // Within 1 day
          );
          
          if (existingReminder) {
            console.log(`🔍 Skipping duplicate smart reminder for ${pattern.category} (already exists)`);
            reminders.push(existingReminder);
            continue;
          }
